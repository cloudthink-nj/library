...
import com.ibroadlink.library.base.utils.BLZipUtils
import okhttp3.*
import okio.Buffer
...
class LogInterceptor : Interceptor {
    ...
    companion object {
        ...
        fun isJson(mediaType: MediaType?): Boolean {
            return if (mediaType?.subtype() == null) {
                false
            } else mediaType.subtype().lowercase().contains("json")
        }

        fun isXml(mediaType: MediaType?): Boolean {
            return if (mediaType?.subtype() == null) {
                false
            } else mediaType.subtype().lowercase().contains("xml")
        }

        fun isHtml(mediaType: MediaType?): Boolean {
            return if (mediaType?.subtype() == null) {
                false
            } else mediaType.subtype().lowercase().contains("html")
        }
