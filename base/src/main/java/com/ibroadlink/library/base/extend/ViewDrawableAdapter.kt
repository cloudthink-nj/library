...
fun setDrawableIntoView(
    ...
    var count = 0
    val defaultDrawable = drawable_default?.let {
        if (radius == INVALID.toFloat())
            it
        else
            createDrawableCorner(it, radius)
    } ?: create(
        shapeMode,
        solidColor,
        strokeColor,
    ...
    val checkedDrawable = drawable_checked?.let {
        if (radius == INVALID.toFloat())
            it
        else
            createDrawableCorner(it, radius)
    ...
    val checkableDrawable = drawable_checkable?.let {
        if (radius == INVALID.toFloat())
            it
        else
            createDrawableCorner(it, radius)
    ...
    val enabledDrawable = drawable_enabled?.let {
        if (radius == INVALID.toFloat())
            it
        else
            createDrawableCorner(it, radius)
    ...
    val focusedDrawable = drawable_focused?.let {
        if (radius == INVALID.toFloat())
            it
        else
            createDrawableCorner(it, radius)
    ...
    val pressedDrawable = drawable_pressed?.let {
        if (radius == INVALID.toFloat())
            it
        else
            createDrawableCorner(it, radius)
    ...
    val selectedDrawable = drawable_selected?.let {
        if (radius == INVALID.toFloat())
            it
        else
            createDrawableCorner(it, radius)
    ...
    if (count < 1) {
        ...
        if (isDefaultNull || count == 1) {
            ...
            TMP_PADDING[0] = view.paddingLeft
            TMP_PADDING[1] = view.paddingTop
            TMP_PADDING[2] = view.paddingRight
            TMP_PADDING[3] = view.paddingBottom
        }
        if (count == 1 && !isDefaultNull) {
            intoView(view, defaultDrawable)
        } else {
...
private fun create(
    ...
    if (shapeMode == INVALID && solidColor == null && strokeColor == INVALID
        && strokeWidth == INVALID.toFloat() && strokeDash == INVALID.toFloat() && strokeDashGap == INVALID.toFloat()
        && radius == INVALID.toFloat() && radiusLT == INVALID.toFloat() && radiusLB == INVALID.toFloat()
        && radiusRT == INVALID.toFloat() && radiusRB == INVALID.toFloat()
        && startColor == INVALID && centerColor == INVALID && endColor == INVALID
        && orientation == INVALID && gradientType == INVALID && radialCenterX == null
        && radialCenterY == null && radialRadius == INVALID.toFloat() && width == INVALID.toFloat()
        && height == INVALID.toFloat() && marginLeft == INVALID.toFloat() && marginTop == INVALID.toFloat()
        && marginRight == INVALID.toFloat() && marginBottom == INVALID.toFloat()
    ) {
...
private fun setRingValue(
    ...
    try {
        ...
        if (innerRadiusRatio != null) {
            ...
            mInnerRadiusRatio.setFloat(
                mGradientState[drawable],
                dip2px(innerRadiusRatio).toFloat()
            )
        }
...
private fun createDrawableCorner(drawable: Drawable?, radius: Float): Drawable? {
    val toRoundCorner =
        ImageUtils.toRoundCorner(ImageUtils.drawable2Bitmap(drawable), dip2px(radius).toFloat())
    return ImageUtils.bitmap2Drawable(toRoundCorner)
}

private fun intoView(view: View, drawable: Drawable?) {
    // add fix ImageView和ImageButton改为setImageDrawable
    when (view) {
        is ImageView -> {
