...
fun String?.isNumeric(): Boolean {
    return this?.let {
        Pattern.matches(it.trim(), RegexConstants.REGEX_INTEGER)
    } ?: let {
        false
    }
}
 ...
 * 是否为邮箱号
 */
fun String?.eachContains(string: String?): Boolean {
    return notNull(this, string) {
        this!!.contains(string!!) || string.contains(this);
    } ?: let {
        false
    }
}